    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="About" format="csv" />
                <livewire:excel-export model="About" format="xlsx" />
                <livewire:excel-export model="About" format="pdf" />
            @endif
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('about_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.abouts.show', $about) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('about_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.abouts.edit', $about) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('about_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $about->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
